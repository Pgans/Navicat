...
DATE_FORMAT(NOW(), '%Y%m%d%H%i%s') AS D_UPDATE , ipd_reg.DRG, ipd_reg.RW,  ipd_reg.ADJRW, '' ERROR , '' WARNING,  '' ACTLOS , '' GROUPER_VERSION, cid_hn.CID
FROM 
...
INNER JOIN mbase_data.opd_diagnosis ON opd_diagnosis.VISIT_ID = opd_visits.VISIT_ID and opd_diagnosis.dxt_id = '1' AND opd_diagnosis.IS_CANCEL =0
INNER JOIN mbase_data.main_inscls ON  main_inscls.INSCL = opd_visits.INSCL
LEFT OUTER JOIN mbase_data.refers  on opd_visits.visit_id = refers.visit_id  and refers.rf_type = '1' AND refers.IS_CANCEL = 0 
LEFT OUTER JOIN mbase_data.refers AS rf_out ON opd_visits.visit_id = rf_out.visit_id  and rf_out.rf_type = '2'
LEFT OUTER JOIN mbase_data.receipts ON opd_visits.visit_id = receipts.visit_id AND receipts.IS_CANCEL = 0
