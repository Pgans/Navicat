...
END AS LABRESULT,
DATE_FORMAT(NOW(), '%Y%m%d%H%i%s') AS D_UPDATE, gcoffice.offid LABPLACE, cid_hn.CID
FROM 
	...
	inner join mbase_data.chronic_reg on chronic_reg.cid = population.cid
	right outer join mbase_data.lab_requests on lab_requests.VISIT_ID = opd_visits.visit_id and lab_requests.lab_id IN ('013', '014', '015', '021', '018', '019', '020', '009', '011', '123', '016', '299', '168', '147') 
...
WHERE  LEFT( population.cid, 5 ) > 00000 
	AND opd_visits.reg_datetime BETWEEN '2018.01.01 00:00' AND '2018.01.15 23:59' 
